...
#xia edit: remove Z
###ExactGibbsT = function(X, E, Z, gammaZ, b0, v, r, s, qr, nburn=2000, niter=4000){
ExactGibbsT = function(X, E, gammaZ, b0, v, r, s, qr, nburn=2000, niter=4000){
  n <- nrow(X)
  p <- ncol(X)
  tau0_sq= b0/n
...
# Xia edit: initialize Z
 ...
 for(itr in 1:(nburn+niter)){
   ...
   sigma1 = solve( t(X)%*%diag(1/Omega_sq)%*%X+ D + eps_diag)
   ...
   for(i in 1:n){
     ...
     ### UPDATE Omega_sq
       Omega_sq[i] = rinvgamma(1, shape = (v+1)/2, rate = (v + (Z[i] - sum(X[i,]*beta))^2 )/2)
   }
